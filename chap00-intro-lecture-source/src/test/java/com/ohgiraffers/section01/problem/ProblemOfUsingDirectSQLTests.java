...
//            menu.setMenuPrice(rset.getInt("MENU_PRICE"));
     ...
     *  ex) 일반회원, 관리자의 개념이 있을 때 회원의 서브타입으로 일반회원, 관리자를 별도로 분리
     *      (자바는 상속의 개념으로 실제로 물려주지만 DB는 join이다.)
    * */

    /* 설명. 3-2. 연관관계 문제, 객체 그래프 탐색 문제, 방향성 문제
     *  객체지향에서 말하는 가지고 있는 (ASSOCIATION 연관관계 혹은 COLLECTION 연관관게) 경우 데이터베이스 저장 구조와
     *  다른 형태이다.
     *
     * 필기.
    ...
    * */
     ...
     *  menu1 == menu2  -> true(동일성이 보장된다. 같은 객체)
    * */
}

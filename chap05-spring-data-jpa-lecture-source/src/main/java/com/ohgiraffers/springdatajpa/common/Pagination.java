...
public class Pagination {
    ...
    public static PagingButtonInfo getPagingButtonInfo(Page page) {
        ...
        int dafaultButtonCount = 10;                // 한 페이지에 보일 버튼 최대 갯수
        int startPage;                              // 한 페이지에 보여질 첫 버튼
        int endPage;                                // 한 페이지에 보여질 마지막 버튼

        /* 필기. ceil -> 소수점이 있으면 올림 처리 0.1 -> 1 */
        startPage = (int)(Math.ceil((double) currentPage / dafaultButtonCount) - 1) * dafaultButtonCount + 1;
        endPage = startPage + dafaultButtonCount - 1;
